...
export function TransferForm({
  ...
  const [depotBalance, setDepotBalance] = useState(0);
  ...
  useEffect(() => {
    if (!productId) return;
    getDepotBalanceAction(productId).then(setDepotBalance);
  }, [productId, state]);
  ...
  const volumeMax = usesVolumeUnits
    ? maxDisplayQuantity(depotBalance, volumeUnit)
    : depotBalance || undefined;
    ...
    <form action={formAction} className="space-y-4">
      ...
      {productId && (
        <Badge variant="secondary" className="w-full justify-center py-2">
          Available at Depot:{" "}
          {formatStockAvailability(depotBalance, selectedProduct)}
        </Badge>
      )}
